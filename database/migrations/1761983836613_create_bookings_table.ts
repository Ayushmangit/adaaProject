...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      table.increments('id')
      //TODO: add service_id relation as well
      table.integer('service_slot_info_id').references('service_slot_infos.id').nullable()
        .onDelete('SET NULL') //TODO: onDelete("cascade")
      table.integer('service_membership_info_id').references('service_membership_infos.id').nullable().onDelete('SET NULL') //TODO: onDelete("cascade")
      table.integer('user_id').references('users.id').onDelete('CASCADE')
      table.timestamp('start_at').notNullable()
      //table.enu('payment_mode',['upi','cash'])
      //table.boolean('payment_status').defaultTo('false')

      //TODO: table.unique(["service_slot_info_id", 'service_membership_info_id', start_at]) 
    ...
    `) // TODO: check if this can be implimented with orm only
  }
